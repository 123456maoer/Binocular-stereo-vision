...
def left_projective_stn(x, theta):
    shape = x.size()
    # orig_img = x.clone() #TODO - Remove
    tx = torch.linspace(-1,1,shape[2]).unsqueeze(0).repeat(shape[2],1)
    ty = torch.linspace(-1,1,shape[3]).unsqueeze(1).repeat(1,shape[3])
...
class ConfigNet(nn.Module):
    def __init__(self, stereo_model, stn_mode='projective', ext_disp2depth=False, right_head=False):
        ...
        self.left_localization = nn.Sequential(
            nn.Conv2d(3, 8, kernel_size=7),
            nn.MaxPool2d(2, stride=2),
            nn.ReLU(True),
            nn.Conv2d(8, 10, kernel_size=5),
        ...
        self.left_fc_loc = nn.Sequential(
            nn.Linear(36000, 36),
            nn.Linear(36, 32),
            nn.ReLU(True),
        ...
        self.left_fc_loc[3].weight.data.fill_(0)
        self.left_fc_loc[3].weight.data.zero_()
        if self.stn_mode == 'projective':
            self.left_fc_loc[3].bias.data.copy_(torch.tensor([0, 0, 0, 0, 0], dtype=torch.float))
    ...
    def left_stn(self, x):
        x,theta1 = left_projective_stn(x, self.left_theta(x))
        # x,theta1 = stn(x, self.theta(x),mode=self.stn_mode)
        return x, theta1
    ...
    def left_theta(self, x):
        xs = self.left_localization(x)
        xs = xs.view(-1, 36000)
        theta = self.left_fc_loc(xs)
        # theta = torch.unsqueeze(theta,0)
        return theta

    def forward(self, left_img, right_img):
        # transform the input
        right_img_transformed, theta_right = self.stn(right_img)
        left_img_transformed, theta_left = self.left_stn(left_img)
        if self.right_head:
            if self.training:
                _,_,stereo_out,_,_,right_stereo_out = self.stereo_model(left_img_transformed, right_img_transformed)
            else:
                stereo_out, right_stereo_out,_ = self.stereo_model(left_img_transformed, right_img_transformed)
        else:
            if self.training:
                _,_,stereo_out = self.stereo_model(left_img_transformed, right_img_transformed)
            else:
                _, stereo_out = self.stereo_model(left_img_transformed, right_img_transformed)
        ...
        if self.right_head:
            return stereo_out, theta_right, right_img_transformed,right_stereo_out
        else:
            return stereo_out, theta_right, theta_left, right_img_transformed,left_img_transformed
