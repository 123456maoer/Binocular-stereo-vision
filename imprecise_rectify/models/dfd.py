...
class Dfd_net(nn.Module):
    ...
    def forward(self, x,focal_point=0.7,D=2.28*1e-3):
        x = self.conv1(x)#3 2160 3840
        x = self.batch_norm1(x)#16 2160 3840
        x = F.relu(x)#16 2160 3840
        x = self.pool1(x)#16 1080 1920
        x = F.relu(self.batch_norm2(self.conv2(x)))#36 1080 1920
        x = self.pool2(x)#36 540 960
        x = F.relu(self.batch_norm3(self.conv3(x)))#64 540 960
        x = self.pool3(x)#64 270 480
        skip8 = x
        x = F.relu(self.batch_norm4(self.conv4(x)))#128 270 480
        x = self.pool4(x)#128 135 240
        skip = x
        x = F.relu(self.batch_norm5(self.conv5(x)))#256 135 240
        x = self.pool5(x)#256 67 120
        x = self.conv6(x)#16 67 120
        if self.mode == 'segmentation':
            x = self.upsampling(x) #16 2176 3872
            if self.skip_layer:
