...
class ConfigNet(nn.Module):
    ...
    def forward(self, left_img, right_img):
        # transform the input
        left_img_transformed, theta = self.stn(left_img)
        if self.right_head:
            if self.training:
                _,_,stereo_out,_,_,right_stereo_out = self.stereo_model(left_img_transformed, right_img)
            else:
                stereo_out, right_stereo_out,_ = self.stereo_model(left_img_transformed, right_img)
        else:
            if self.training:
                _,_,stereo_out = self.stereo_model(left_img_transformed, right_img)
            else:
                _, stereo_out = self.stereo_model(left_img_transformed, right_img)
        ...
        if self.right_head:
            return stereo_out, theta, left_img_transformed,right_stereo_out
        else:
            return stereo_out, theta, left_img_transformed
