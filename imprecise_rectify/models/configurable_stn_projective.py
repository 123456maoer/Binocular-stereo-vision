...
def projective_stn(x, theta):
    ...
    tx = torch.linspace(-1,1,shape[3]).unsqueeze(0).repeat(shape[2],1)
    ty = torch.linspace(-1,1,shape[2]).unsqueeze(1).repeat(1,shape[3])
    ...
    try:
        x = F.grid_sample(x, grid[:,:,:,:2], align_corners=True)
    except:
        x = F.grid_sample(x, grid[:,:,:,:2], align_corners=True)
...
def projective_stn2(x, theta):
    ...
    #This is due to cudnn bug
    try:
        x = F.grid_sample(x, grid[:,:,:,:2], align_cormers=True)
    except:
        x = F.grid_sample(x, grid[:,:,:,:2], align_cormers=True)
    # import matplotlib.pyplot as plt
    # print(theta1)
...
def stn(x, theta, mode='affine'):
    ...
    x = F.grid_sample(x, grid, align_cormers=True)
    return x, theta1


class ConfigNet(nn.Module):
    def __init__(self, stereo_model, stn_mode='projective', ext_disp2depth=False, right_head=False,device=None, tune_5_params=True):
        ...
        batch_size = 1
        self.tune_5_params = tune_5_params
        # self.theta_var = Variable(torch.zeros([batch_size, 5], dtype=torch.float32, device=device), requires_grad=True)
        if tune_5_params:
            self.theta_var = nn.Parameter(torch.zeros([batch_size, 5], dtype=torch.float32, device=device), requires_grad=True)
        # self.theta_var = nn.Parameter(torch.zeros([batch_size, 8], dtype=torch.float32, device=device), requires_grad=True)
        else:
            self.theta_var = nn.Parameter(torch.Tensor([1,0,0,1,0,0,0,0]).repeat(batch_size,1), requires_grad=True)
        # self.theta_var[:, 0] += 1
        # self.theta_var[:, 3] += 1
        # self.theta_var[:, 0] = nn.Parameter(torch.ones([batch_size, 1], dtype=torch.float32, device=device), requires_grad=True)
        # self.theta_var[:, 3] = nn.Parameter(torch.ones([batch_size, 1], dtype=torch.float32, device=device), requires_grad=True)
    ...
    def stn(self, x):
        ...
        #
        # x,theta1 = projective_stn(x, self.theta_var)
        if self.tune_5_params:
            x,theta1 = projective_stn(x, self.theta_var)
        else:
            x, theta1 = projective_stn2(x, self.theta_var)
    ...
    def forward(self, left_img, right_img):
        # transform the input
        right_img_transformed, theta = self.stn(right_img) #右图变换结果和变换矩阵
        if self.right_head:
            if self.training:
...
if __name__ == '__main__':
    ...
    stereo_model.cuda()
    c = ConfigNet(stereo_model=stereo_model, tune_5_params=True)
    print ("A")