...
from models.dfd import Dfd_net

def batched_softmax(input_tensor, batch_size):
        num_channels = input_tensor.size(1)
        num_batches = (num_channels + batch_size - 1) // batch_size

        batches = torch.split(input_tensor, batch_size, dim=1)
        softmax_results = []

        for batch in batches:
            softmax_result = F.softmax(batch, dim=1)
            softmax_results.append(softmax_result)

        merged_softmax = torch.cat(softmax_results, dim=1)
        return merged_softmax
class hourglass(nn.Module):
    ...
    def forward(self, x ,presqu, postsqu):
        ...
        # print("out、pre：",out.shape, pre.shape)
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        # print("cost0:",cost0.shape,"cost1:",cost1.shape,"cost2:",cost2.shape,"cost3:",cost3.shape)
        if self.right_head:
        ...
        if self.training:
            ...
            pred1 = disparityregression_calib(self.maxdisp)(pred1)

            cost2 = torch.squeeze(cost2,1)
            pred2 = F.softmax(cost2,dim=1)
            pred2 = disparityregression_calib(self.maxdisp)(pred2)

            if self.right_head:
                cost4 = F.interpolate(cost4, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                                   align_corners=True)
                cost5 = F.interpolate(cost5, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                                   align_corners=True)

                cost4 = torch.squeeze(cost4, 1)
                pred4 = F.softmax(cost4, dim=1)
                pred4 = disparityregression_calib(self.maxdisp)(pred4)

                cost5 = torch.squeeze(cost5, 1)
                pred5 = F.softmax(cost5, dim=1)
                pred5 = disparityregression_calib(self.maxdisp)(pred5)

            if self.pred_oof:
                cost7 = F.interpolate(cost7, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                                   align_corners=True)
                cost8 = F.interpolate(cost8, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                                   align_corners=True)

                cost7 = torch.squeeze(cost7, 1)
                pred7 = F.softmax(cost7, dim=1)
                pred7 = disparityregression_calib(self.maxdisp)(pred7)

                cost8 = torch.squeeze(cost8, 1)
                pred8 = F.softmax(cost8, dim=1)
                pred8 = disparityregression_calib(self.maxdisp)(pred8)
        ...
        # print(cost3.shape)  
                # 应用分批 softmax
        cost3_shape = cost3.size()
        batch_size = 32  # 根据显存调整
        pred3 = batched_softmax(cost3, batch_size)  
        
        # pred3 = F.softmax(cost3,dim=1)
        conf192 = pred3
        pred3 = disparityregression_calib(self.maxdisp)(pred3)
        torch.cuda.empty_cache() 
        if self.right_head:
            cost6 = F.interpolate(cost6, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                               align_corners=True)
            cost6 = torch.squeeze(cost6, 1)
            pred6 = F.softmax(cost6, dim=1)
            pred6 = disparityregression_calib(self.maxdisp)(pred6)

        if self.pred_oof:
            cost9 = F.interpolate(cost9, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear',
                               align_corners=True)
            cost9 = torch.squeeze(cost9, 1)
            pred9 = F.softmax(cost9, dim=1)
            pred9 = disparityregression_calib(self.maxdisp)(pred9)
